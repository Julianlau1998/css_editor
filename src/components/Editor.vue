...
export default {
    ...
    methods: {
        ...
        createFileLink (fileName) {
            const file = new Blob([this.code], { type: "data:text/csv;charset=utf-8" }, `${fileName}.css`)
        ...
        downloadFile (fileName) {
            ...
            hiddenElement.download = `${fileName}.css`
            hiddenElement.click();
        ...
        highlighter(code) {
          if (Prism.languages.css) {
            this.css = Prism.languages.css
          }
          return highlight(code, this.css, ''); // languages.<insert language> to return html with markup
        },
    }
}
</script>
